...
angular.module( 'isteven-multi-select', ['ng'] ).directive( 'istevenMultiSelect' , [ '$sce', '$timeout', '$templateCache', function ( $sce, $timeout, $templateCache ) {
    return {
        ...
        link: function ( $scope, element, attrs ) {                       
                    ...
                    angular.forEach( $scope.inputModel, function( value, key ) {
                        if ( typeof value !== 'undefined' && value[ attrs.tickProperty ] === true ) {                        
                            if ( ctr < tempMaxLabels ) {                            
                              //  $scope.varButtonLabel += ( $scope.varButtonLabel.length > 0 ? '</div>, <div class="buttonLabel">' : '<div class="buttonLabel">') + 'Kathir';
                              $scope.varButtonLabel = 'Dynamic Parameters';
                            }
                            //ctr++;
                        }
                    });                
            ...
            $scope.icon.tickMarkUnchecked   = '&#9745;';    // a tick icon 
            $scope.icon.tickMarkChecked   = '&#9744;';    // a tick icon 
            ...
            if ( typeof attrs.translation !== 'undefined' ) {
                ...
                $scope.lang.reset           = $sce.trustAsHtml( $scope.icon.reset      + '&nbsp;&nbsp;Reset Selection' );
                $scope.lang.search          = 'Please Type Your Search Parameters';
                $scope.lang.nothingSelected = 'Selected None';                
            }
            $scope.icon.tickMarkUnchecked = $sce.trustAsHtml( $scope.icon.tickMarkUnchecked );
            $scope.icon.tickMarkChecked = $sce.trustAsHtml( $scope.icon.tickMarkChecked );
        ...
        '<span class="multiSelect inlineBlock">' +
            ...
            '<div class="checkboxLayer show">' +
                ...
                '<div class="checkBoxContainer">'+
                    ...
                    '<div class="acol">'+
                        '<label>'+    
                        '<span class="tickMarkChecked" ng-if="item[ groupProperty ] !== false && item[ tickProperty ] === false" ng-bind-html="icon.tickMarkChecked"></span>'+                            
                        '<span class="tickMarkUnchecked" ng-if="item[ groupProperty ] !== true && item[ tickProperty ] === true" ng-bind-html="icon.tickMarkUnchecked"></span>'+
...
angular.module( 'isteven-multi-show', ['ng'] ).directive( 'istevenMultiShow' , [ '$sce', '$timeout', '$templateCache', function ( $sce, $timeout, $templateCache ) {
    return {
        restrict: 
            'AE',
    ...
    var template = 
        ...
        '</div>'+
    '</span>';
    $templateCache.put( 'isteven-multi-show.htm' , template );
}]); 
